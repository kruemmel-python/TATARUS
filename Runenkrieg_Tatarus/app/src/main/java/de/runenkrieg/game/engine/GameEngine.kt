...
class GameEngine(
    ...
    fun continueAfterReveal(state: GameState): GameState {
        ...
        if (gameOver) {
            val winnerText = when {
                ...
                state.aiTokens > state.playerTokens -> "TATARUS gewinnt dieses Duell."
    ...
    private fun resolveRound(state: GameState, playerCard: RuneCard): GameState {
        ...
        val winnerText = when (winner) {
            ...
            Winner.AI -> "TATARUS gewinnt Runde ${state.round}"
            Winner.DRAW -> "Runde ${state.round} endet unentschieden"
        }
        val learningText = if (decision.wasExploration) {
            "TATARUS hat eine neue Antwort erkundet."
        } else {
            "TATARUS nutzte seine bisher beste Strategie."
        }
        return state.copy(
            playerHand = playerHand,
            aiHand = aiHand,
            ...
            status = "$winnerText · Stärke Du/TATARUS $scoreText. $learningText",
    ...
    private fun applyMechanics(
        ...
        fun apply(card: RuneCard, playerOwned: Boolean, won: Boolean, remainingHand: List<RuneCard>) {
            ...
            val actor = if (playerOwned) "Deine Karte" else "TATARUS"
