...
data class EvaluationModeResult(
    ...
    val roundWins: Int = 0,
    val roundDraws: Int = 0,
    val roundLosses: Int = 0
) {
    val winRate: Double
        get() = if (games == 0) 0.0 else wins.toDouble() / games

    val roundWinRate: Double
        get() {
            val total = roundWins + roundDraws + roundLosses
            return if (total == 0) 0.0 else roundWins.toDouble() / total
        }
}
