...
class TatarusAi(
    ...
    refreshFromAsset: Boolean = false
) : GameOpponent {
    ...
    init {
        if (loadPersistedState && initialAssetName != null &&
            (refreshFromAsset || !modelFile.exists())
        ) {
            modelFile.parentFile?.mkdirs()
            context.applicationContext.assets.open(initialAssetName).use { input ->
                FileOutputStream(temporaryModelFile).use { output ->
                    input.copyTo(output)
                }
            }
            runCatching {
                Files.move(
                    temporaryModelFile.toPath(),
                    modelFile.toPath(),
